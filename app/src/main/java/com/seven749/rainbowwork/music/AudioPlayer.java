...
public class AudioPlayer implements MediaPlayer.OnCompletionListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnBufferingUpdateListener, MediaPlayer.OnErrorListener {
    ...
    public void onCompletion(MediaPlayer mp) {
        BasePlayReceiver.sendBroadcastCompletion(App.getInstance());
    }

    @Override
    public void onPrepared(MediaPlayer mp) {
        start();
        BasePlayReceiver.sendBroadcastPrepared(App.getInstance());
    }

    @Override
    public void onBufferingUpdate(MediaPlayer mp, int percent) {
        BasePlayReceiver.sendBroadcastBufferingUpdate(App.getInstance(), percent);
    }
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        ...
        release();
        next();
        BasePlayReceiver.sendBroadcastError(App.getInstance(), what, extra);
        return false;
    }
    ...
    public void init() {
        ...
        // 使用唤醒锁
        mMediaPlayer.setWakeMode(App.getInstance(), PowerManager.PARTIAL_WAKE_LOCK);
        ...
        // 初始化wifi锁
        wifiLock = ((WifiManager) App.getInstance().getApplicationContext().getSystemService(Context.WIFI_SERVICE)).createWifiLock(WifiManager.WIFI_MODE_FULL, "mylock");
        // 初始化音频焦点管理器
        audioFocusManager = new AudioFocusManager(App.getInstance());
    ...
    private void play(AlbumProgramItemBean source) {
        ...
        // 发送初始化资源信息的广告
        BasePlayReceiver.sendBroadcastInitSource(App.getInstance(), source);
        // 获取音频地址（音频地址一般私有）
        String id = source.getId();
        String url = "http://47.99.165.194/song/detail?ids=" + id;
...
//        mMediaPlayer.reset();
    ...
    private void start() {
        ...
        EventBus.getDefault().post(new MainActivityEvent());
        // 发送播放状态的广播
        BasePlayReceiver.sendBroadcastPlayStatus(App.getInstance());
    }
